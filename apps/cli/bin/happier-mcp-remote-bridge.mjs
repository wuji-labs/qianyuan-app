...
if (!hasNoWarnings || !hasNoDeprecation) {
  ...
  const entrypoint = await prepareRuntimeEntrypoint(projectRoot, join('mcp', 'bridges', 'remoteMcpStdioBridge.mjs'));
  ...
  const projectRoot = dirname(dirname(fileURLToPath(import.meta.url)));
  import(await prepareRuntimeEntrypoint(projectRoot, join('mcp', 'bridges', 'remoteMcpStdioBridge.mjs')));
}
