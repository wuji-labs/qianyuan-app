...
type DownloadInitRequest = Readonly<{
  path: string;
  asZip?: boolean;
...
export function registerDownloadTransferHandlers(
  ...
  registerDownloadTransferLifecycleHandlers<DownloadInitResponse>({
    rpcHandlerManager,
    store: deps.store,
    methods: {
      init: RPC_METHODS.FILES_DOWNLOAD_INIT,
      chunk: RPC_METHODS.FILES_DOWNLOAD_CHUNK,
      finalize: RPC_METHODS.FILES_DOWNLOAD_FINALIZE,
      abort: RPC_METHODS.FILES_DOWNLOAD_ABORT,
    },
    resolveInit: async (data) => {
      const request = data as DownloadInitRequest | null;
