 ...
 * agent loop. Resume paths previously synthesized the catch-up cursor from `session.seq`, so user
 * rows committed while the runner was down were never delivered on any resume. The runner persists
 * this watermark in session metadata; daemon attach paths clamp the catch-up cursor to it so owed
 * rows are redelivered (at-least-once, deduped by localId/echo suppression).
 */
