...
description: Use the Agents surface to launch, monitor, and interact with Happier subagents, Claude teams, and provider-backed agent threads.
---
...
- **Happier subagents**: review, plan, and delegate runs started by Happier
...
- launch from the **Launch Happier subagents** card
...
Claude team agents appear in the same Agents roster as Happier subagents, but their controls are Claude-specific.
...
### Launch Happier subagents

Use **Launch Happier subagents** when you want Happier to start a review, plan, or delegate run.
...
- Happier subagent transcripts stream and update through the same session data flow
...
While a Happier subagent is running, you can send follow-up instructions from:
...
- starting a Happier subagent resumes the session first, then launches the subagent
...
- Happier treats it as **observe-only** for Happier subagents
...
Open **Agents** → **Launch Happier subagents** → **Review**.
...
Start a delegate-style Happier subagent, then keep using the row or details pane to steer it while it runs.
