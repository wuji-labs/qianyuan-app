...
| **Server runtime installer** (`https://happier.dev/install-server`) | `curl …install-server \| bash` (macOS/Linux) | You rerun it | No | Re-run the installer to replace the `happier-server` binary (rolling tags `server-stable` / `server-preview` / `server-dev`) |
...
This is a convenience wrapper for server runtime binaries (macOS/Linux). Update by rerunning it:
...
If you installed the **server runtime binary** directly via the installer (macOS/Linux), update by rerunning it:
