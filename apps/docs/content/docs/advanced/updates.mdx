...
| **CLI installer** (`https://happier.dev/install`) | `curl …install \| bash` / `irm …install.ps1` | You rerun it (or use the CLI’s self-update for binary installs) | No | Re-run the installer, or for binary installs run `happier self update` |
| **CLI binary** (installer-based) | Installed under `~/.happier/bin` with a shim in `~/.local/bin` (or Windows equivalent) | You run `happier self update` or rerun the installer | No (but it can show notices) | `happier self check` / `happier self update` (supports `--channel preview`, `--to <versionOrTag>`) |
